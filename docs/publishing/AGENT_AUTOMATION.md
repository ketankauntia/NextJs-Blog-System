...
2. Use Copy setup prompt on `/docs` for the default install, or on the setup page for personalized choices. It copies a short instruction and the absolute `/agent-setup.md` link. The endpoint is public and self-contained. A collapsed fallback copies the full instructions when an agent cannot reach localhost.
