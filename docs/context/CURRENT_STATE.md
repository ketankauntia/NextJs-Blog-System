...
- `/docs` presents AI setup first, then the manual guide. One button copies a short prompt and the absolute `/agent-setup.md` URL. A collapsed fallback provides full instructions when an agent cannot reach the URL. The docs header has no duplicate source, Markdown or AI toolbar.
- `/dashboard/setup` exports validated, non-secret schema-v2 `publishing.json`: Local with a repository-relative content folder, or login-based self-hosting with a custom login route, GitHub/Supabase content and optional R2 assets. Managed and OAuth stay disabled.
...
- `/agent-setup.md?setup=...` serves validated personalized instructions without caching. Invalid choices return 400. The agent waits for required provider sign-in and explicit go-ahead, verifies account/resources and carries out authorized setup. It asks for unavailable access and decisions it cannot infer.
