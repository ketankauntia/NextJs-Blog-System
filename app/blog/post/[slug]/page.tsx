...
export async function generateMetadata({
  ...
  // Explicit ogImage wins, then the visible cover, then the card generated by
  // opengraph-image.tsx alongside this file.
  const socialImage = post.ogImage
    ...
    : post.coverImage
      ...
      : { url: `${url}/opengraph-image`, width: 1200, height: 630, type: "image/png" };
  ...
  const resolvedTitle = buildTitle([post.title]);
  const resolvedDescription = buildDescription(post.description, [
    `Published on ${siteConfig.name}${post.category ? ` under ${post.category}` : ""}`,
  ]);
  return {
