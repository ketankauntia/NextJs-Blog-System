...
function SeoScoreBadge({ score }: { score: number }) {
  const tone = score >= 80 ? "good" : score >= 55 ? "warn" : "low";
  return <span className={`content-seo-score is-${tone}`} aria-label={`SEO score ${score} percent`} title="Local editorial checks currently passing">
    <span>SEO</span><strong>{score}%</strong>
  </span>;
}

export function DashboardClient({ rows, readOnly = false, view = "list", initialReviews = {}, today }: {
  rows: PostRow[]; readOnly?: boolean; view?: "list" | "board"; initialReviews?: Record<string, ReviewRecord>; today: string;
}) {
  ...
  return <main id="main-content" className="content-library">
    ...
    {selected && <aside className="content-detail" aria-label="Selected article details">
      ...
      <div className="content-detail-seo"><SeoScoreBadge score={selected.seoScore} /><span>Local editorial checks passing</span></div>
