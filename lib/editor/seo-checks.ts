...
/** True when `url` points back at this site, so self-links are not counted as citations. */
function isOwnDomain(url: string): boolean {
  try {
    return new URL(url).hostname === new URL(siteConfig.url).hostname;
  } catch {
    return false;
  }
}
...
export function runSeoChecks(draft: DraftInput): SeoCheck[] {
  ...
  checks.push({ id: "internal-links", group: "seo", label: "Internal links", status: internal >= 1 ? "pass" : "warn", detail: internal >= 1 ? `${internal} internal link${internal === 1 ? "" : "s"}` : "Link to at least one related post to build the topic map" });
    ...
    detail:
      bodySections.length === 0
        ...
        : weakOpenings.length === 0
          ? "Every section opens with a direct answer, which is ideal for AI extraction"
  ...
  checks.push({
    ...
    detail:
      statTotal >= statTarget
        ...
        : `Add ${statTarget} concrete stat${statTarget === 1 ? "" : "s"} (a %, count, or measured number). Statistics are the strongest GEO lever.`,
  });
    ...
    checks.push({
      ...
      detail: Number.isNaN(ageDays)
        ...
        : ageDays <= 365
          ...
          : `Last updated ${ageDays} days ago. Refresh it and bump the date because AI engines favor recent content`,
    });
  }

  // ---- Structure group (GEO blocks) ----
  const tldrLen = draft.tldr.trim().length;
  checks.push({ id: "tldr", group: "structure", label: "TL;DR summary", status: tldrLen === 0 ? "fail" : tldrLen >= 150 && tldrLen <= 500 ? "pass" : "warn", detail: tldrLen === 0 ? "Missing. This is the highest-leverage block for AI citations" : `${tldrLen} chars. Aim for 150–500 (2–4 answer-first sentences)` });
  ...
  checks.push({ id: "no-h1", group: "structure", label: "No H1 in body", status: hasH1 ? "fail" : "pass", detail: hasH1 ? "Body contains a `# ` heading. The post title is the only H1, so use `## `" : "Title is the only H1" });
  ...
  // ---- Readability group ----
  checks.push(range("word-count", "readability", "Word count", words, [600, 100000], [300, 100000], `${words} words. Use 600+ for a substantive post, 300 minimum`));
